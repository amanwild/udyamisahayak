...
if (($_SERVER["REQUEST_METHOD"] == "POST") && isset($_POST["addList"]) && $_POST['listing_title']) {
  ...
  $listing_firm_name = "";
  if (isset($_POST["listing_firm_name"])) {
    $listing_firm_name = $_POST['listing_firm_name'];
  }
 ...
 include "../service/db.php";
  try {
    $insert_query = "INSERT INTO `listing` (`listing_id`, `listing_business_name`,`listing_firm_name`, `listing_business_category_id`, `listing_keyword`, `listing_tag`, `listing_cost`, `listing_business_sub_category_id`, `listing_description`, `listing_country_id`, `listing_pincode`, `listing_state_id`, `listing_location`, `listing_city_id`, `listing_image`,`listing_image2`,`listing_image3`, `listing_timestamp`, `listing_status_timestamp`, `listing_status`, `listing_permission`, `listing_owner_id`) VALUES (NULL, '$listing_title','$listing_firm_name', '$category_id', '$keywords', '$tags', '$price', '$sub_category_id', '$listing_description', '1', '$pincode', '$state_id', '$address', '$city_id', '$logo_image1','$logo_image2','$logo_image3',  current_timestamp(),  current_timestamp(), 'Pending', 'Pending', '" . $_SESSION['user_id'] . "')";
            ...
            <div id="utf_add_listing_part">
              <form method="POST" action="<?= $_SERVER["REQUEST_URI"]; ?>">
                ...
                <div class="add_utf_listing_section margin-top-45" validate>
                    ...
                    <div class="col-md-6">
                      ...
                      <div class="intro-search-field utf-chosen-cat-single">
                        <select class="selectpicker " name="category_id" id="category" data-selected-text-format="count" data-size="7" title="Select Category" required>
                          ...
                          $select_category_query = "SELECT * FROM `category` order by category_id desc"; //NOTE: here (`) is not equal to (')
                          try {
                  ...
                  <div class="row with-forms">
                    <div class="col-md-6">
                      <label for="listing_firm_name">Company Name</label>
                      <input type="text" class="" name="listing_firm_name" id="listing_firm_name" placeholder="" required />
                    </div>
                    <div class="col-md-6">
