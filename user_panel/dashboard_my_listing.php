...
// echo  $select_query;
                while ($row = mysqli_fetch_assoc($select_result)) {
                  ...
                  $listing_adderess = $row['listing_location'];
                  $listing_city_id = $row['listing_city_id'];
                  $listing_image = $row['listing_image'];
                  $listing_image2 = $row['listing_image2'];
                  $listing_image3 = $row['listing_image3'];
                  $listing_since = $row['listing_timestamp'];
                  $listing_permission_timestamp = $row['listing_permission_timestamp'];
                  $listing_status_timestamp = $row['listing_status_timestamp'];
                  ...
                  $select_listing_sub_category_query = "SELECT * FROM sub_category WHERE sub_category_id = $listing_sub_category_id";
                  try {
                    $select_listing_sub_category_result = 0;
                    if ($connect) {
                      $select_listing_sub_category_result = mysqli_query($connect, $select_listing_sub_category_query);
                      if ($select_listing_sub_category_result) {
                        $select_listing_sub_category_num = mysqli_num_rows($select_listing_sub_category_result);
                      }
                    }
                  } catch (Exception $e) {
                    $mess = $e->getMessage();
                  }
                  if ($select_listing_sub_category_num > 0) {
                    $sno = 0;
                    //Note : mysqli_fetch_assoc($result) it returns NULL when no data is persent to Select
                    while ($row_for_listing_sub_category = mysqli_fetch_assoc($select_listing_sub_category_result)) {
                      $listing_sub_category_name = $row_for_listing_sub_category['sub_category_name'];
                      // $listing_sub_category_color = $row_for_listing_sub_category['category_color'];
                      // $listing_sub_category_image = $row_for_listing_sub_category['category_image'];
                    }
                  }
                  // echo $listing_rating;
                ...
                ?>
                  <li class="tableItem utf_approved_booking_listing">
                    <div class="utf_list_box_listing_item ">
                      <div class="utf_list_box_listing_item-img"><a href="#"><img src="../images/<?= $listing_image ?>" alt=""></a></div>
                        ...
                        <div class="inner row">
                          <div class=" col-md-6" style="">
                            ...
                            if ($listing_status == 'Expired') {
                              ...
                              <span><i class="fa fa-building"></i> Company : <?= $listing_firm_name ?> </span>
                            <span><i class="fa fa-folder"></i> Category : <?= $listing_category_name ?> </span>
                            <span><i class="fa fa-folder"></i> Sub Category : <?= $listing_sub_category_name ?> </span>
                            <span><i class="fa fa-map-marker"></i><?= $listing_adderess ?></span>
                            <span><i class="fa fa-link"></i> <?= $user_email ?></span>
